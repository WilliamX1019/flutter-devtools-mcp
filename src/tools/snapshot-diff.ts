...
export function registerSnapshotDiffTools(
  ...
  server.registerTool(
    "compare_snapshots",
    {
      ...
      inputSchema: {
        before: z.string().describe('Name of the "before" snapshot'),
        after: z.string().describe('Name of the "after" snapshot'),
      },
    },
    async ({ before, after }) => {
      ...
      const heapDiff = snap2.memory.heapUsage - snap1.memory.heapUsage;
      const capacityDiff = snap2.memory.heapCapacity - snap1.memory.heapCapacity;

      const beforeMap = new Map(snap1.memory.topClasses.map((c) => [c.name, c]));
      const afterMap = new Map(snap2.memory.topClasses.map((c) => [c.name, c]));

      const allClassNames = new Set([...beforeMap.keys(), ...afterMap.keys()]);
      ...
      if (heapDiff < -1_000_000) {
        ...
        output.push("➡️ No significant change in memory usage between snapshots.");
      }
