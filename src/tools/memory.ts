 ...
 */
export function registerMemoryTools(server: McpServer, client: FlutterVmServiceClient) {
    ...
    async ({ forceGC, topN }) => {
      ...
      try {
        ...
        const appClasses = sortedByInstances.filter(
          (m) =>
            m.instancesCurrent > 0 && m.bytesCurrent > 0 && !isVmInternal(m.class.name)
        );
        ...
        const suspiciousClasses = appClasses.filter((m) => m.instancesCurrent > 500);
